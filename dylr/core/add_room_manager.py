...
def try_add_room(info):
    try:
        ...
        logger.error("添加主播失败，请确认输入的内容是否符合要求\n如果符合要求，可能是接口失效，请换种方式。")
...
def find_by_web_rid(web_rid):
    ...
    if room is not None:
        logger.error(f"重复获取主播房间: {room.room_name}({web_rid})")
        if app.win_mode:
    ...
    req = requests.get(
        api_url, headers=dy_api.get_request_headers(), proxies=dy_api.get_proxies()
    )
    ...
    logger.info(f"成功获取到房间{name}({web_rid})")
    if app.win_mode:
...
def find_user(info: str):
    ...
    if web_rid is not None:
        ...
        room = Room(
            f"将会在开播时获取{random.randint(1, 100000000)}",
            nickname,
            True,
            True,
            False,
