...
class DanmuRecorder:
        ...
        for msg in payloadPackage.messagesList:
            if msg.method == "WebcastChatMessage":
                ...
                with open(self.filename, "a", encoding="UTF-8") as file:
                    file.write(
                        f'  <d p="{round(second, 2)},1,25,16777215,'
        ...
        while True:
            ...
            if t % 10 == 0:
                ...
                if self.retry < 3 and self.danmu_amount == 0 and t > 30:
                    ws.close()
                    logger.warning(
                        f"{self.room_name}({self.room_id}) 无法获取弹幕，正在重试({self.retry + 1})"
                    )
                    # time.sleep(5)
    ...
    def _onClose(self, ws, a, b):
        ...
        with open(self.filename, "a", encoding="UTF-8") as file:
            file.write("</i>")
        logger.info(f"{self.room_name}({self.room_id}) 弹幕录制结束")
        if app.stop_all_threads:
            return
        if self.retry < 10 and dy_api.is_going_on_live(self.room):
            self.retry += 1
            logger.info(f"{self.room_name}({self.room_id})弹幕录制重试({self.retry})")
