...
def get_api_url(room_id):
    return (
        "https://live.douyin.com/webcast/room/web/enter/?aid=6383&live_id=1&device_platform=web&language=zh-CN"
...
def get_live_state_json(room_id):
    ...
    try:
        ...
        logger.debug(
            f"failed to load response of GET to json when finding stream url of {room_id}, using api 1, "
            f"response: " + res
        )
        cookie_utils.record_cookie_failed()
        return None
    try:
        ...
        logger.debug(
            f"failed to load json when finding stream url of {room_id}, using api 1, "
            f"response: " + res
        )
        cookie_utils.record_cookie_failed()
        return None
...
def get_danmu_ws_url(room_id, live_room_real_id, retry=0):
    ...
    if index == -1:
        logger.warning(
            f"cannot find user_unique_id when loading RENDER_DATA of {room_id}"
        )
        return get_danmu_ws_url(room_id, live_room_real_id, retry + 1)
...
def get_user_info(sec_user_id):
    ...
    headers = {
        ...
        + "; msToken="
        + ms_token
        + "; odin_tt=324fb4ea4a89c0c05827e18a1ed9cf9bf8a17f7705fcc793fec935b637867e2a5a9b8168c885554d029919117a18ba69; passport_csrf_token=f61602fc63757ae0e4fd9d6bdcee4810;",
    }
