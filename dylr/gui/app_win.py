...
class ApplicationWin(ttk.Frame):
    ...
    def init_win(self):
        ...
        add_room_btn = tk.Button(
            footer_frame, text="添加主播", font=("微软雅黑", 14), command=self._request_add_room
        )
    ...
    def reload_all(self):
        ...
        for room in record_manager.get_rooms():
            self.grip_frame.append(
                room.room_id,
                room.room_name,
                "未直播" if room.auto_record else "未监测",
                room.auto_record,
                room.record_danmu,
                room.important,
            )
    ...
    def _request_add_room(self):
        res = simpledialog.askstring(
            title="添加主播",
            prompt="请输入房间地址，支持Web_Sid、直播间地址、直播间分享短链、主播主页，如\n"
            "123456\n"
